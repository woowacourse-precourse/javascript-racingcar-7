...
describe("자동차 경주 게임 입력값 유효성 검사", () => {
  ...
  describe("validateCarCount", () => {
    test("자동차가 1대일 경우 에러를 던진다", () => {
      expect(() => validateCarCount(["car1"])).toThrow(
        "[ERROR] 경주에 참가할 자동차는 2대 이상이어야 합니다."
      );
    });

    test("자동차가 0대일 경우 에러를 던진다", () => {
      expect(() => validateCarCount([])).toThrow(
        "[ERROR] 경주에 참가할 자동차는 2대 이상이어야 합니다."
      );
    });
  ...
  describe("validateDuplicate", () => {
    test("중복된 자동차 이름이 있으면 에러를 던진다", () => {
      expect(() => validateDuplicate(["car1", "car1"])).toThrow(
        "[ERROR] 중복된 자동차 이름이 있습니다."
      );
      expect(() => validateDuplicate(["car1", "car2", "car1"])).toThrow(
        "[ERROR] 중복된 자동차 이름이 있습니다."
      );
    });
  ...
  describe("validateCarNameLength", () => {
    test("자동차 이름이 비어있으면 에러를 던진다", () => {
      expect(() => validateCarNameLength("")).toThrow(
        "[ERROR] 자동차 이름은 1자 이상이어야 합니다."
      );
    });

    test("자동차 이름이 5자를 초과하면 에러를 던진다", () => {
      expect(() => validateCarNameLength("car123")).toThrow(
  ...
  describe("validateNumberRound", () => {
    test("시도 횟수가 숫자가 아니면 에러를 던진다", () => {
      expect(() => validateNumberRound("abc")).toThrow(
        "[ERROR] 시도 횟수는 숫자여야 합니다."
      );
      expect(() => validateNumberRound("123abc")).toThrow(
        "[ERROR] 시도 횟수는 숫자여야 합니다."
      );
    });

    test("시도 횟수가 숫자이면 에러를 던지지 않는다", () => {
      expect(() => validateNumberRound(5)).not.toThrow();
      expect(() => validateNumberRound("5")).not.toThrow();
      expect(() => validateNumberRound(10)).not.toThrow();
    });
  });

  describe("validatePositiveRound", () => {
    test("시도 횟수가 1 미만이면 에러를 던진다", () => {
      expect(() => validatePositiveRound(0)).toThrow(
        "[ERROR] 시도 횟수는 1 이상이어야 합니다."
      );
      expect(() => validatePositiveRound(-1)).toThrow(
        "[ERROR] 시도 횟수는 1 이상이어야 합니다."
      );
    });

    test("시도 횟수가 1 이상이면 에러를 던지지 않는다", () => {
      expect(() => validatePositiveRound(1)).not.toThrow();
      expect(() => validatePositiveRound(5)).not.toThrow();
      expect(() => validatePositiveRound(100)).not.toThrow();
    });
  });
});
