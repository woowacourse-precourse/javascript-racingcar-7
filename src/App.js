...
class App {
  ...
  validateCarNames(cars) {
    ...
    if (this.isDuplicateCarName(cars)) {
      throw Error(`[ERROR] ${App.ERROR_MESSAGE.CAR_NAME_DUPLICATE_ERROR}`);
    }
  }
  ...
  isDuplicateCarName(cars) {
    const carsNames = cars.map(car => car.name);
    const uniqueNames = new Set(carsNames);
    return carsNames.length !== uniqueNames.size;
  }
}
