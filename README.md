...
- [ ] 자동차 입력 유효성 검사 테스트
  - [ ] 입력받은 자동차의 길이가 5자보다 길면 에러를 반환한다.
  - [ ] 중복된 자동차의 이름이 있으면 에러를 반환한다.
- [ ] 입력받은 라운드 유효성 검사 테스트
  - [ ] 숫자가 아니면 에러를 반환한다
  - [ ] 정수가 아니면 에러를 반환한다
  - [ ] 0 이하의 숫자를 받으면 에러를 반환한다
  - [ ] infinity나 nan과 같은 값이라면 에러를 반환한다.
- [ ] 무작위 숫자를 생성할 때 무작위 숫자가 4 이상이면 자동차가 움직인다.
- [ ] 자동차 테스트
  - [ ] 자동차가 움직이면 자동차의 거리가 늘어난다.
- [ ] 자동차의 거리 출력이 포맷과 일치한다.
- [ ] 우승자 리스트가 포맷에 맞게 출력된다.
