...
#### 확장
- 차 이름이 한국어일 경우
  - 정규표현식을 수정하여 한국어를 입력받도록 만들면 될 것 같음.
- 전진할 수 있는 주사위 숫자가 입력으로 주어진다면
  - 주사위를 굴리는 car 클래스 생성 시 이름과 함께 전진 숫자 받는 방법
  - 전진 자체를 racingCar 클래스에서 하며 racingCar 클래스에 전진 숫자를 받는 방법
  - 전역 변수로 전진 숫자를 선언해버리는 방법
- 우승 조건이 다를 경우
  - 가장 적게 간 사람이 우승하는 경우
  - 가장 멀리간 사람이 아닌 어디 까지 누가 가장 먼저 도착했는지로 해서 순위를 출력하는 게임