...
describe("InputHandler", () => {
    ...
    test("자동차 이름이 5자 이상인 경우 오류 발생", () => {
        ...
        expect(() => inputHandler.processTrimInput(input)).toThrow("[ERROR] 자동차 이름은 5자 이하여야 합니다.");
    });

    test("자동차 이름이 빈 문자열일 경우 오류 발생", () => {
        const inputHandler = new InputHandler();
        const input = "car1,,car3";
        expect(() => inputHandler.processTrimInput(input)).toThrow("[ERROR] 자동차 이름은 빈 문자열일 수 없습니다.");
    });

    test("자동차 이름이 중복될 경우 오류 발생", () => {
        const inputHandler = new InputHandler();
        const input = "car1,car1,car3";
        expect(() => inputHandler.processTrimInput(input)).toThrow("[ERROR] 자동차 이름은 중복될 수 없습니다.");
    });

    test("자동차가 1개일 경우 오류 발생", () => {
        const inputHandler = new InputHandler();
        const input = "car1";
        expect(() => inputHandler.processTrimInput(input)).toThrow("[ERROR] 자동차는 최소 2대 이상이어야 합니다.");
    });
});